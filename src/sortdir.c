...
struct dirent *readdir(DIR *d) {
    ...
    s = slot_for(d, 1);
    if (!s) { pthread_mutex_unlock(&g_lock); return real_readdir ? real_readdir(d) : NULL; }
    if (!s->loaded && load_stream(s, d) != 0) { s->loaded = 1; s->pos = s->count; }
...
struct dirent64 *readdir64(DIR *d) {
    ...
    pthread_mutex_lock(&g_lock);
    s = slot_for(d, 1);
    if (!s) { pthread_mutex_unlock(&g_lock); return real_readdir64(d); }
    if (!s->loaded && load_stream(s, d) != 0) { s->loaded = 1; s->pos = s->count; }
...
int closedir(DIR *d) {
    ...
    if (s) {
        ...
        free(s->ents);
        s->dir = NULL; s->ents = NULL; s->count = 0; s->pos = 0; s->loaded = 0;
    }
    pthread_mutex_unlock(&g_lock);
